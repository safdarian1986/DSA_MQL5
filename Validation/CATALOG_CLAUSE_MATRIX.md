...
| Feature engineering | causal feature snapshot, selected channel contract, and sequence signatures | PASS | selection, volatility, structure, memory, cycle, MTF, and sequence inputs are covered by feature and sequence harnesses. |
...
| Machine Learning | Adaptive AR-Ridge plus feature engine | PASS | bounded AR-Ridge, adaptive lambda approval, feature/lag/time weighting, and ML mode are covered. |
...
| 17 | Adaptive Historical Weight | PASS | Ridge uses causal time decay and adaptive lambda approval; sequence uses age decay | Keep bounded windows. |
...
| 22 | Kalman Local Linear Trend | PASS | local-linear level/slope update uses innovation and quality-scaled gain with model-detail proof | Keep causal state. |
| 23 | Adaptive AR-Ridge | PASS | bounded 5D Ridge uses lag/channel features, time decay, quality-scaled regularization, and adaptive lambda approval | Keep Fast Path inference-only. |
...
| 27 | Feature Engine | PASS | price, return, candle, volatility, volume, memory, cycle, structure, MTF, and sequence-relevant features are implemented and tested | Keep features causal. |
